...
def draw_full_graph():
    global ms, timer_line, time_list

    time_list       = beatmap_parser.file_times(osu_lines)
    difficulty_list = diff_calc.rate_change_difficulty(time_list)[2]
    ...
    for i in difficulty_list:
        canvas_difficulty_list.append(height - i * (height/2))

    full_graph_canvas.create_rectangle(0, 0, width, height, fill="#dddddd")
    full_graph_canvas.create_line(0, height*0.2, width, height*0.2, fill="#cccccc")
    full_graph_canvas.create_line(0, height*0.4, width, height*0.4, fill="#cccccc")
    full_graph_canvas.create_line(0, height*0.6, width, height*0.6, fill="#cccccc")
    full_graph_canvas.create_line(0, height*0.8, width, height*0.8, fill="#cccccc")
    full_graph_canvas.create_line(width*0.2, 0, width*0.2, height, fill="#cccccc")
    full_graph_canvas.create_line(width*0.4, 0, width*0.4, height, fill="#cccccc")
    full_graph_canvas.create_line(width*0.6, 0, width*0.6, height, fill="#cccccc")
    full_graph_canvas.create_line(width*0.8, 0, width*0.8, height, fill="#cccccc")

    # Draw blue line graph, difficulty.
    for i in range(len(difficulty_list) - 1):
        # Don't continue the graph if there is a break.
        if time_list[i + 1] - time_list[i] < 3000:
            full_graph_canvas.create_line(canvas_time_list[i], canvas_difficulty_list[i],
    ...
    for n in range(len(time_break_separated_list)):
        for x in range(len(time_break_separated_list[n]) - average_values):
            ...
            if time_list[i + 1 + int(average_values/2)] - time_list[i + int(average_values/2)] < 3000:
                full_graph_canvas.create_line(canvas_time_list[i + int(average_values/2)],
    ...
    # Draw timer line.
    timer_line = full_graph_canvas.create_line(0, 0, 0, 150, fill="#77ff77")

# Function used to draw the full graph timer.
def draw_full_graph_timer():
    global ms, timer_line, time_list
    ...
    if ms < time_list[-1]:
        ...
        full_graph_canvas.coords(timer_line, draw_x, 0, draw_x, height)

    time_label.after(30, draw_full_graph_timer)
...
while True:
    ...
    Button(root, fg="blue", text="Start Realtime!", command=start_map).pack()
    ...
    full_graph_canvas = Canvas(root, width=600, height=200)
    full_graph_canvas.pack()
    ...
    Label(root, text="Total Delay Difficulty: {:.4f}\nTotal Rate Change Difficulty: {:.4f}".format(total_delay_difficulty, total_rate_change_difficulty)).pack()
